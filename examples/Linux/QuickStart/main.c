...
void CallbackGetAnglesExt(void *param) {
	i32 frameCamAngleTemp =
		sbgcDegreeToAngle(GetAnglesExt.AxisGAE[PITCH].frameCamAngle);

	/* If the callback argument was NULL, the library defaults to passing a
	 * pointer to the SBGC32_Device object */
	DebugSBGC32_PrintStructElement((sbgcGeneral_t *)param, &frameCamAngleTemp,
								   "Current Angle Pitch =", sbgcINT);
}
...
/* Let's consider several operational modes */
	...
	SBGC32_Control(&SBGC32_Device, &Control, SBGC_DEFAULT_FEATURES_, &tokenCtrl,
				   SBGC_NO_CALLBACK_);
	/* The SBGC32_Control function doesn't automatically process the
	 * confirmation command, so it must be registered manually */
	SBGC32_CheckConfirmation(&SBGC32_Device, &Confirm, CMD_CONTROL,
							 SBGC_DEFAULT_FEATURES_, &tokenCtrl,
							 SBGC_NO_CALLBACK_);

	/* Register usual commands */
	SBGC32_ReadBoardInfo(&SBGC32_Device, &BoardInfo, 0, SBGC_DEFAULT_FEATURES_,
						 &tokenBI, SBGC_NO_CALLBACK_);
	SBGC32_ReadParams3(&SBGC32_Device, &MainParams3, sbgcCURRENT_PROFILE,
					   SBGC_DEFAULT_FEATURES_, &tokenMP3, SBGC_NO_CALLBACK_);

	/* Register the serial commands that will not be deleted */
	SBGC32_GetAnglesExt(
		&SBGC32_Device, &GetAnglesExt, SCParam_RETAIN | SCParam_NO_NEED_LOG,
		SCPrior_LOW, SCTimeout_DEFAULT, &tokenGAE, CallbackGetAnglesExt, NULL);
	...
	if (SerialAPI_GetConfirmStatus(&Confirm) != sbgcCONFIRM_RECEIVED) {
		DebugSBGC32_PrintMessage(
			&SBGC32_Device,
			"No confirmation of control detected! Terminating the program...");
	 ...
	 */
	SBGC32_GetAnglesExt(&SBGC32_Device, &GetAnglesExt, SCParam_NO, SCPrior_LOW,
						SCTimeout_DEFAULT, &tokenGAE, SBGC_NO_CALLBACK_);
	...
	/* This command will enter the user callback after successful reception */
	SBGC32_ReadRealTimeData4(&SBGC32_Device, &RealTimeData,
							 SBGC_DEFAULT_FEATURES_, CallbackReadRealTimeData,
							 NULL);

	/* The SBGC32_Control function doesn't automatically process the
	 * confirmation command, so it must be registered manually */
	SBGC32_Control(&SBGC32_Device, &Control, SBGC_DEFAULT_FEATURES_,
				   SBGC_NO_CALLBACK_);
	SBGC32_CheckConfirmation(&SBGC32_Device, &Confirm, CMD_CONTROL,
							 SCParam_FORCE_CALLBACK, SCPrior_LOW,
							 SCTimeout_DEFAULT, SBGC_NO_CALLBACK_);
	...
	SBGC32_GetAnglesExt(&SBGC32_Device, &GetAnglesExt,
						SCParam_RETAIN | SCParam_NO_NEED_LOG, SCPrior_LOW,
						SCTimeout_DEFAULT, CallbackGetAnglesExt, NULL);
	...
	/* In blocking mode, the buffer automatically compresses to hold only one
	 * command */
	/* A registered command is processed immediately with a total wait time of
	 * SBGC_DEFAULT_TIMEOUT */
	/* Configure gimbal to further control */
	SBGC32_ControlConfig(&SBGC32_Device, &ControlConfig, &Confirm);
	...
	if (SBGC32_ReadRealTimeData4(&SBGC32_Device, &RealTimeData) ==
		sbgcCOMMAND_OK) {
		DebugSBGC32_PrintMessage(&SBGC32_Device, "Realtime data received!\n");
	...
	/* The SBGC32_Control function doesn't automatically process the
	 * confirmation command, so it must be received manually */
	...
	/* Get the current PITCH angle over a period of SBGC_GET_ANGLES_TIMEOUT */
	while ((sbgcGetTick() - timestamp) < SBGC_GET_ANGLES_TIMEOUT) {
		if (SBGC32_GetAnglesExt(&SBGC32_Device, &GetAnglesExt) ==
			sbgcCOMMAND_OK) {
			frameCamAngleTemp =
				sbgcDegreeToAngle(GetAnglesExt.AxisGAE[PITCH].frameCamAngle);
